  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors duration-300 flex-shrink-0 ${on ? 'bg-violet-600' : 'bg-slate-200'}`}
      >
...
function InputField({ label, id, type = 'text', placeholder = '', register, error, valueAsNumber }: InputFieldProps) {
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={id} className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{label}</label>
      <div className="relative">
        <input
          id={id}
          type={isPassword && !show ? 'password' : type}
          placeholder={placeholder}
          className={`w-full px-4 py-2.5 rounded-xl bg-slate-50 border text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 transition-all text-sm pr-10 ${
            error ? 'border-red-300 focus:ring-red-500/20 focus:border-red-500' : 'border-slate-200 focus:ring-violet-500/20 focus:border-violet-500'
          }`}
...
function SelectField({ label, id, options, register, error }: SelectFieldProps) {
  return (
    <div className="space-y-1.5">
      ...
      <select
        id={id}
        className={`w-full px-4 py-2.5 rounded-xl bg-slate-50 border text-slate-900 focus:bg-white focus:outline-none focus:ring-2 transition-all text-sm ${
          error ? 'border-red-300 focus:ring-red-500/20 focus:border-red-500' : 'border-slate-200 focus:ring-violet-500/20 focus:border-violet-500'
        }`}
        {...register(id)}
      >
...
export default function AdminSettingsPage() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 animate-fade-in pb-12 max-w-7xl mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Settings</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Configure platform behavior, pricing, and security</p>
        </div>
        <button
          type="submit"
          disabled={saving || !isValid}
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-white bg-gradient-to-r from-violet-600 to-brand-600 hover:from-violet-500 hover:to-brand-500 hover:shadow-glow hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 text-sm font-display shadow-lg"
        >
          {saving ? (
      ...
      <div className="bg-white p-6 rounded-2xl border border-red-100 shadow-sm">
        <div className="flex items-start gap-3 mb-5">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-500 to-rose-700 flex items-center justify-center flex-shrink-0">
            <Shield className="w-5 h-5 text-white" />
          </div>
          <div>
            <h2 className="font-bold text-red-600 text-lg">Danger Zone</h2>
            <p className="text-xs text-slate-500 mt-0.5">Irreversible actions that affect the entire platform</p>
          </div>
        </div>
        <div className="space-y-3">
          {[
            { label: 'Reset all surge pricing',     desc: 'Disable surge in all zones and reset multipliers to 1.0x',  btnColor: 'border-amber-200 text-amber-700 hover:bg-amber-50 bg-amber-50/20' },
            { label: 'Force logout all users',       desc: 'Invalidate all active sessions across the platform',        btnColor: 'border-red-200 text-red-700 hover:bg-red-50 bg-red-50/20' },
            { label: 'Purge ride history (30+ days)',desc: 'Permanently delete ride records older than 30 days',         btnColor: 'border-red-200 text-red-700 hover:bg-red-50 bg-red-50/20' },
          ].map((action, i) => (
            <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-red-50/10 border border-red-100">
              <div className="min-w-0 pr-4">
              ...
              <button
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold border transition-all ${action.btnColor}`}
              >
                Execute
              </button>
            </div>
